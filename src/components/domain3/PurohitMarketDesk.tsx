  ...
  const handleBookGigSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updates[`communities/${session.communityId}/purohit_anushthans/${anuId}`] = {
        ...
        yajamanPhone: '', // Captured later or from profile
      ...
      addNotification({
        ...
        message: `Your booking for ${selectedGig.title} with ${selectedGig.purohitName} has been confirmed.`,
        type: 'success',
      });
  ...
  return (
    <div className="space-y-6 fade-in pb-12 relative w-full flex flex-col min-h-[90vh]">
      ...
      <div className="bg-gradient-to-br from-indigo-900 via-blue-900 to-indigo-950 p-8 sm:p-12 rounded-3xl shadow-2xl relative overflow-hidden ring-1 ring-black/10 text-white flex flex-col justify-center min-h-[250px]">
        ...
        <div className="relative z-10 max-w-3xl">
          <span className="text-[10px] font-black uppercase tracking-widest bg-blue-500/30 text-blue-200 px-3 py-1 rounded-full border border-blue-400/30 shadow-inner mb-4 inline-block">
            {safeTranslate('verified_network', 'Verified Scholar Network')}
          </span>
          <h2 className="text-3xl sm:text-4xl font-black mb-3 tracking-tight leading-tight">
            {safeTranslate('marketplace_title', 'Find the Perfect Vedic Scholar for your Rituals')}
          </h2>
          <p className="text-sm font-medium text-blue-200 mb-8 max-w-xl leading-relaxed">
            {safeTranslate('marketplace_subtitle', 'Hire verified experts with transparent ratings, upfront pricing, and guaranteed Sanatani authenticity.')}
          </p>
          ...
          <div className="relative w-full max-w-2xl flex items-center bg-white rounded-2xl p-1 shadow-2xl">
            ...
            <input 
              type="text" 
              placeholder={safeTranslate('search_rituals', "What service are you looking for? (e.g. Vastu Shanti)")}
              value={searchTerm} 
            ...
            <button className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-md hidden sm:block shrink-0">
              {safeTranslate('btn_search', 'Search')}
            </button>
          </div>
        </div>
      </div>
        ...
        {activeTab === 'GIGS' && (
          <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-hide px-2">
            ...
            {['ALL', 'Mandir & Home Rituals', 'Special Seva', 'Off-site Seva'].map((cat, idx) => (
              ...
              >
                {cat === 'ALL' ? safeTranslate('filter_all', 'All Categories') : cat}
              </button>
            ))}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredGigs.length > 0 ? (
                  ...
                  <div className="p-6 pt-8 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex flex-col">
                        <span className="text-sm font-black text-gray-900 flex items-center gap-1.5 hover:underline">
                          {gig.purohitName}
                          {gig.verifiedBadge && <ShieldCheck size={14} className="text-blue-500" />}
                        </span>
                    ...
                    <div className="mt-auto border-t border-gray-100 pt-4 flex justify-between items-center bg-white">
                      ...
                      <div className="text-right">
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">{safeTranslate('starting_at', 'STARTING AT')}</p>
              ...
              <div className="col-span-full py-20 text-center bg-white rounded-3xl border border-dashed border-gray-300 shadow-sm">
                ...
                <p className="text-xl font-black text-gray-800 mb-1">{safeTranslate('no_gigs', 'No services found.')}</p>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{safeTranslate('adjust_filters', 'Try adjusting your search criteria.')}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeTab === 'MY_ORDERS' && (
        <div className="space-y-6 animate-in fade-in max-w-5xl mx-auto w-full">
          <div className="flex items-center justify-between border-b border-gray-200 pb-4 px-2">
            <div>
              <h3 className="text-xl font-black text-gray-900 flex items-center gap-2"><FileText className="text-gray-700"/> {safeTranslate('my_orders', 'My Active Orders')}</h3>
          ...
          <div className="space-y-4">
            {contracts.length > 0 ? (
              ...
              <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300 shadow-sm">
                ...
                <p className="text-xl font-black text-gray-800 mb-1">{safeTranslate('no_contracts', 'No active orders found.')}</p>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{safeTranslate('explore_gigs_desc', 'Explore the marketplace to book a ritual.')}</p>
              </div>
            )}
          </div>
        </div>
            ...
            {checkoutStep === 'DETAILS' && (
              <div className="overflow-y-auto flex-1 bg-white scrollbar-hide flex flex-col md:flex-row pb-24 md:pb-0">
                   ...
                   <div>
                     ...
                     <div className="flex flex-wrap items-center gap-4 text-sm">
                       <div className="flex items-center gap-2 font-black text-gray-800">
                         ...
                         {selectedGig.verifiedBadge && <ShieldCheck size={14} className="text-blue-500" />}
                       </div>
                       <span className="text-gray-300">|</span>
                ...
                <div className="w-full md:w-80 bg-gray-50 md:bg-white p-6 sm:p-8 flex flex-col justify-start md:sticky md:top-0 h-auto md:h-full border-t md:border-t-0 border-gray-200 fixed bottom-0 left-0 right-0 z-20 md:z-auto">
                   ...
                   >
                     {safeTranslate('continue_checkout', 'Continue to Checkout')} <ChevronRight size={18}/>
                   </button>
                </div>
              </div>
            )}
      ...
      <div className="pt-12 pb-6 text-center opacity-70 border-t border-gray-200 mt-auto text-xs font-bold text-gray-500 shrink-0">
        Made with <Heart size={12} className="text-red-500 fill-current inline"/> by <span className="font-black text-blue-500">TrackIQ Academy</span> • Global Purohit Marketplace
      </div>
    </div>
  );
}
