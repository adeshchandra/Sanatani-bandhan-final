...
export const MasterSettingsDesk: React.FC = () => {
  const { activeWorkspace, updateWorkspaceDetails, currentRole, currentUser } = useAuthWorkspace();
  ...
  const isAdmin = currentRole === 'superadmin' || currentRole === 'trustee';
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'banner'|'logo') => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) return showToast(safeTranslate('err_only_images', "Only images allowed."), "error");
  ...
  const handleSaveWorkspace = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdmin) return showToast(safeTranslate('err_unauthorized', "Unauthorized. Admins only."), "error");
    ...
    try {
      ...
      showToast(safeTranslate('settings_saved', "Workspace Settings Updated Successfully!"), "success");
    } catch (err: any) {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (passwordData.newPassword.length < 6) return showToast(safeTranslate('err_pass_length', "Password must be at least 6 characters."), "error");
    if (passwordData.newPassword !== passwordData.confirmPassword) return showToast(safeTranslate('err_pass_match', "Passwords do not match."), "error");

    setConfirmDialog({
      title: safeTranslate('update_password', "Update Admin Password"),
      message: safeTranslate('confirm_pass_update', "Are you sure you want to change your secure master password?"),
      confirmText: safeTranslate('btn_update_pass', "UPDATE PASSWORD"),
      isDanger: true,
      onConfirm: async () => {
        ...
        try {
          ...
          if (user) {
            ...
            showToast(safeTranslate('pass_updated_success', "Secure Password Updated Successfully!"), "success");
  ...
  const handleSubmitUpgrade = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setConfirmDialog({
        title: safeTranslate('req_submitted', "Request Submitted!"),
        message: safeTranslate('dakshina_submitted_desc', "✅ Your transaction ID has been securely submitted. Our backend team will verify this within 24 hours and activate your SMART PRO status."),
        confirmText: safeTranslate('understood', "UNDERSTOOD"),
        isDanger: false,
  ...
  const handleCopyNumber = (num: string) => {
    ...
    setCopied(true);
    showToast(safeTranslate('copied_success', "Number Copied Successfully!"), "success");
        ...
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 sm:p-8 animate-in zoom-in-95 ring-1 ring-white/20 text-center border-t-4 border-sanatani-orange">
            ...
            <div className="flex gap-3">
              {confirmDialog.isDanger && <button onClick={() => setConfirmDialog(null)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-black py-3.5 rounded-xl text-xs uppercase tracking-widest transition-colors shadow-sm">{safeTranslate('btn_cancel', 'Cancel')}</button>}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-6 bg-white p-5 sm:p-6 rounded-3xl shadow-sm ring-1 ring-black/5">
        <div>
          <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2 tracking-tight">
            <Settings className="text-sanatani-orange" size={28} /> {safeTranslate('nav_settings', 'Workspace Settings')}
          </h2>
          <p className="text-xs text-gray-500 font-bold mt-1 uppercase tracking-widest">{safeTranslate('settings_subtitle', 'Manage identity, usage counters, and secure access.')}</p>
        </div>
        {!isAdmin && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
             ...
             <div className="px-6 sm:px-8 pb-8">
               ...
               <h3 className="text-sm font-black text-gray-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 border-b border-gray-100 pb-4 uppercase tracking-widest">
                 <div className="flex items-center gap-2">
                   <Building2 size={18} className="text-sanatani-orange" /> {safeTranslate('official_identity', 'Official Identity')}
                 </div>
               ...
               <form onSubmit={handleSaveWorkspace} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5">{workspaceInfo.type} {safeTranslate('name', 'Name')} *</label>
                    ...
                    <div className="relative">
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1">{safeTranslate('org_type', 'Organization Type')}</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Globe size={12}/> {safeTranslate('operating_currency', 'Operating Currency')}</label>
                    ...
                    <div className="relative">
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1"><MapPin size={12}/> {safeTranslate('country', 'Country')}</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div className="relative">
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Phone size={12}/> {safeTranslate('official_phone', 'Official Phone')}</label>
                    ...
                    <div className="relative">
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Mail size={12}/> {safeTranslate('official_email', 'Official Email')}</label>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="w-full">
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1"><MapPin size={12}/> {safeTranslate('physical_address', 'Physical Address')}</label>
                  ...
                  <div className="border-t border-gray-100 pt-6 mt-6">
                    <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-5 flex items-center gap-1.5"><Briefcase size={14}/> {safeTranslate('legal_info', 'Legal & Extended Information')}</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5">{safeTranslate('reg_no', 'Registration No.')}</label>
                      ...
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5">{safeTranslate('tax_id', 'Tax ID (PAN/BIN/EIN)')}</label>
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1"><FileSignature size={12}/> {safeTranslate('mission_desc', 'Mission / Description')}</label>
                  ...
                  {isAdmin && (
                    <div className="flex justify-end pt-4">
                      <button type="submit" disabled={saving} className="w-full sm:w-auto bg-gray-900 hover:bg-black text-white font-black py-4 px-8 rounded-xl text-xs uppercase tracking-widest flex justify-center items-center gap-2 shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:transform-none">
                        {saving ? <Loader2 size={16} className="animate-spin"/> : <Save size={16}/>} {safeTranslate('btn_save_settings', 'SAVE SETTINGS')}
                      </button>
                    </div>
                  )}
               </form>
          ...
          {isAdmin && (
            <div className="bg-white border border-gray-100 p-6 sm:p-8 rounded-3xl shadow-sm ring-1 ring-black/5">
              <h3 className="text-sm font-black text-gray-900 flex items-center gap-2 mb-6 border-b border-gray-100 pb-4 uppercase tracking-widest">
                ...
                {safeTranslate('global_footprint', 'Global Footprint & Geolocation')}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-1 space-y-4">
                  <p className="text-[11px] font-bold text-gray-500 leading-relaxed uppercase tracking-widest">
                    {safeTranslate('geo_desc', 'Pinpoint your organization\'s exact global coordinates. This enables nearby devotee discovery and localized panchang timings.')}
                  </p>
                  <button
                  ...
                  >
                    ...
                    {isDetectingLocation 
                      ? safeTranslate('detecting', 'Detecting...') 
                      : safeTranslate('detect_location_btn', 'Detect My Location')}
                  </button>
                </div>
                  ...
                  {coords ? (
                    <div className="relative z-10 flex flex-col items-center bg-white/90 backdrop-blur-md p-5 rounded-2xl shadow-lg border border-gray-200/50">
                      ...
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">
                        {safeTranslate('coords_locked', 'Coordinates Locked')}
                      </p>
                    ...
                    <div className="relative z-10 flex flex-col items-center text-gray-400">
                      ...
                      <p className="text-xs font-black uppercase tracking-widest">
                        {safeTranslate('no_coords', 'No Coordinates Set')}
                      </p>
                    </div>
                  )}
                </div>
          ...
          {isAdmin && (
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden ring-1 ring-black/5">
              ...
              <h3 className="text-sm font-black text-gray-900 flex items-center gap-2 mb-6 border-b border-gray-100 pb-4 uppercase tracking-widest"><Shield size={18} className="text-blue-600" /> {safeTranslate('admin_security', 'Admin Security')}</h3>
               <form onSubmit={handleChangePassword} className="space-y-5">
                  <div>
                    <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5">{safeTranslate('new_master_pass', 'New Master Password')}</label>
                    <input type="password" required placeholder={safeTranslate('min_6_char', "Min 6 characters")} value={passwordData.newPassword} onChange={e=>setPasswordData({...passwordData, newPassword:e.target.value})} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-900 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all shadow-sm" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5">{safeTranslate('confirm_pass', 'Confirm Password')}</label>
                    <input type="password" required placeholder={safeTranslate('retype_pass', "Retype password")} value={passwordData.confirmPassword} onChange={e=>setPasswordData({...passwordData, confirmPassword:e.target.value})} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-900 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all shadow-sm" />
                  </div>
                  <div className="flex justify-end pt-2">
                    <button type="submit" disabled={passLoading} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-black py-4 px-8 rounded-xl text-xs uppercase tracking-widest flex justify-center items-center gap-2 shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:transform-none">
                      {passLoading ? <Loader2 size={16} className="animate-spin"/> : <Key size={16}/>} {safeTranslate('btn_update_pass', 'UPDATE PASSWORD')}
                    </button>
                  </div>
               </form>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 ring-1 ring-black/5">
            <h3 className="text-xs font-black text-gray-500 uppercase tracking-widest mb-5 border-b border-gray-100 pb-4 flex items-center gap-2"><Settings size={16}/> {safeTranslate('current_limits', 'Current Limits')}</h3>
            <div className="space-y-6">
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <div className="flex justify-between items-end mb-2.5">
                  <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest flex items-center gap-1.5"><Users size={12}/> {safeTranslate('registered_members', 'Registered Members')}</p>
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <div className="flex justify-between items-end mb-2.5">
                  <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest flex items-center gap-1.5"><FileDigit size={12}/> {safeTranslate('pdfs_generated', 'PDFs Generated')}</p>
          ...
          {!['PREMIUM', 'SMART_PRO'].includes(limits.plan) ? (
             <div className="bg-gradient-to-br from-orange-500 to-red-600 p-6 sm:p-8 rounded-3xl shadow-lg text-white text-center relative overflow-hidden group">
                ...
                <h3 className="text-2xl font-black mb-1 tracking-tight">{safeTranslate('upgrade_title', 'Upgrade to Smart Pro')}</h3>
                ...
                {isAdmin ? (
                  <button onClick={() => { setShowUpgradeModal(true); }} className="w-full bg-white text-sanatani-orange hover:bg-gray-50 font-black py-4 rounded-xl text-xs uppercase tracking-widest transition-all shadow-md hover:shadow-xl hover:-translate-y-0.5 relative z-10 flex items-center justify-center gap-2">
                    {safeTranslate('offer_dakshina', 'OFFER DAKSHINA (UPGRADE)')}
                  </button>
                ) : (
...
fs.writeFileSync('src/components/domain6/MasterSettingsDesk.tsx', content);
