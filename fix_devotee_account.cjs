...
import { Camera, Check, Shield, AlertCircle, Phone, Mail, User, HeartPulse, Sparkles, Save, X, Loader2, Calendar, FileText } from 'lucide-react';
...
export const DevoteeSelfService: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-stone-100 overflow-hidden flex flex-col h-full">
      ...
      <div className="bg-gradient-to-r from-stone-900 to-stone-800 p-6 sm:p-8 flex justify-between items-start text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex gap-6 items-center">
          <div className="relative group shrink-0">
            <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-stone-100 border-4 border-stone-700 shadow-xl overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10">
          {!isEditing ? (
            <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-5 py-2.5 rounded-xl text-sm font-bold text-white transition-all backdrop-blur-sm border border-white/10">
              <User className="w-4 h-4" /> Edit Profile
            </button>
          ) : (
            <div className="flex flex-col sm:flex-row gap-2">
              <button onClick={handleCancel} className="flex items-center justify-center gap-2 bg-stone-700 hover:bg-stone-600 px-5 py-2.5 rounded-xl text-sm font-bold text-white transition-all">
                <X className="w-4 h-4" /> Cancel
              </button>
              <button onClick={handleSave} className="flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 px-5 py-2.5 rounded-xl text-sm font-bold text-stone-900 shadow-lg shadow-amber-500/20 transition-all">
                ...
                {otpSent && (
                  <div className="flex gap-2 mt-2 p-3 bg-stone-50 rounded-xl border border-stone-200 animate-in fade-in">
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-stone-200 rounded-lg text-sm font-bold tracking-widest text-center"
                      placeholder="XXXXXX"
                    />
                    <button onClick={verifyOtp} className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-lg shrink-0 transition-colors">
                      Confirm
                    </button>
                  </div>
                )}
