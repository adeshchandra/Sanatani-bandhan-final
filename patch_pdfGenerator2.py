...
pattern = r"export const generateDevoteeCardPDF = async \([\s\S]*?\} catch \(e\) \{\s*console\.warn\(\"Failed to load avatar for PDF\", e\);\s*\}\s*\}\s*if \(!drawn\) \{[\s\S]*?doc\.save\(`\$\{member\.fullName\.replace\(\/\\s\+\/g, '_'\}_\}_SmartCard\.pdf`\);\s*\};"

replacement = """export const generateDevoteeCardPDF = async (
