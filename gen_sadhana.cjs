...
export default function PersonalSadhanaDesk() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(logsRef, (snap) => {
      ...
      const myLogs = parsedLogs.filter(l => l.uid === currentUser?.id).sort((a,b) => b.timestamp - a.timestamp);
      setLogs(myLogs);
      
      const publicLogs = parsedLogs.filter(l => l.isPublic).sort((a,b) => b.timestamp - a.timestamp);
